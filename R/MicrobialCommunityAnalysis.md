...
### 2.) Meta analysis curation.
  - This is a foundational file that this script relies upon.
  - This file utilizes both a phyloseq object (for ANCOM-BCII) and a traditional file where rows are samples and columns are ASVs. In my metadata file the first 30 columns are metadata columns (Treatment, timepoint, gentoype susceptibility, and so on) from columns 31 to the end of the file are ASVs and their count data. 
